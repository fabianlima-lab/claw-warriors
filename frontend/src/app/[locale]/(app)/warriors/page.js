...
export default function WarriorsPage() {
  ...
  const deployLabel = isProTribe
    ? tribeSelections.length !== 1
      ? t('deployCountPlural', { count: tribeSelections.length })
      : t('deployCount', { count: tribeSelections.length })
    : selectedWarrior
      ? t('deploy', { name: selectedWarrior.name })
      : t('selectWarrior');

  return (
    <div className="max-w-5xl mx-auto px-6 py-12">
      <div className="text-center mb-10">
        <SectionLabel warriorClass={selectedClass}>{t('step')}</SectionLabel>
        <h1 className="font-[family-name:var(--font-display)] text-3xl text-txt mt-3">
          {t('title')}
        </h1>
        {isProTribe && (
          <p className="text-txt-muted mt-2">
            {t('selected', { count: tribeSelections.length, max: maxSelections })}
          </p>
        )}
      </div>
